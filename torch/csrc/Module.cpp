...
:func:`torch.set_num_threads` onto the new thread.
  ...
  py_module.def(
      ...
      [](const py::object& handle, uint64_t value) {
        auto mode = fake_mode_from_capsule(handle);
        TORCH_CHECK(mode != nullptr, "No C++ FakeTensorMode handle provided");
        mode->epoch_ = value;
      },
  ...
  py_module.def(
      ...
      [](const py::object& converter,
        ...
        // handlers (PyInterpreter.cpp) observe the same single identity while it
        // is alive. It must be weak: the Python object owns this C++ mode via
        // the returned capsule, so a strong back-reference would cycle. If it is
        // collected, getFakeModePyObj mints an equivalent wrapper on demand. The
        // mode is NOT registered in TLS here; the Python object installs it only
        // for the duration of a `with cpp_mode:` scope (see _push/_pop).
