...
def _get_profiling_args(node: ir.ExternKernel) -> list[str | None]:
    ...
    if node.arg_properties and isinstance(node.op_overload, torch._ops.OpOverload):
        args = node.fill_non_provided_args(args, kwargs)
    profiling_args = _profiling_arg_entries(args)
    positional_names = {
        arg_property.get("name") for arg_property in node.arg_properties[: len(args)]
    }
