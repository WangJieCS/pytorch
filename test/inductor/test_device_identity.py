...
from torch._inductor.graph import _uses_gpu_cpp_wrapper
...
class TestMultiProcessorCount(TestCase):
    ...
    def test_xpu_override_without_hardware(self):
        with mock.patch.object(
            ...
            return_value=SimpleNamespace(gpu_subslice_count=32),
        ):
...
class TestUsesGpuCppWrapper(TestCase):
    def test_builtin_devices(self):
        self.assertTrue(_uses_gpu_cpp_wrapper("cuda"))
        self.assertTrue(_uses_gpu_cpp_wrapper("xpu"))
        self.assertFalse(_uses_gpu_cpp_wrapper("cpu"))
        self.assertFalse(_uses_gpu_cpp_wrapper("mps"))
